...
各テーブルのデータに対してCRUD処理（作成・参照・更新・削除）を行うことができます。そのため、荒らしコメントや不適切なコメントの削除、荒らしユーザーの削除などに使うことができます。

* 管理画面へ入る方法

管理画面に入ることができるのは、管理者権限を持ったユーザーのみです。管理者権限を持ったユーザーのメールアドレスとパスワードは以下の通りです。

メールアドレス: admin@example.com

パスワード: adminuser

上記メールアドレス・パスワードでログイン後、ヘッダー内の白線で囲まれた部分をクリックし、表示されたメニューボックス内の「管理画面」をクリックすると、管理画面へ遷移します。
